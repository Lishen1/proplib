...
public:
  ...
  std::map<std::string, TestClass_1*> childs;
  ...
  void set_random()
  {
    for (int i = 0; i < 2; i++)
    {
      ...
      childs["test1"] = new TestClass_1();
      childs["test2"] = new TestClass_1();

      childs["test1"]->set_random();
      childs["test2"]->set_random();
    }
...
private:
  ...
  SERIALIZE(childs, "childs childs childs");
  SERIALIZE_SUBS();
};
