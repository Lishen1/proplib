...
namespace editor
{
  ...
  int EditorWindow::setup_window()
  {
    ...
    window_ = std::unique_ptr<GLFWwindow, decltype(window_destroyer_)>(glfwCreateWindow(1280, 720, "Dear ImGui GLFW+OpenGL3 example", NULL, NULL),
                                                                       window_destroyer_);
  ...
  public:
    ...
    void         set(const std::string& new_value) { node[name.data()] = value = new_value; }
  ...
  struct IntGuiElement : GuiElement<int>
  {
    ...
    void set(const int new_value) { node[name.data()] = value = new_value; }
    void makeGui() override
    {
      ImGui::InputInt((name + " - " + type_name).data(), &value);
      ImGui::SameLine();
      HelpMarker(doc.data());
    }
  ...
  struct FloatGuiElement : GuiElement<double>
  {
    ...
    void    makeGui() override
    {
      ImGui::InputDouble((name + " - " + type_name).data(), &value);
      ImGui::SameLine();
      HelpMarker(doc.data());
    }
  ...
  {
    std::vector<T>& get() { return value; }
    void            set(const std::vector<T>& new_value) { node[name.data()] = value = new_value; }
  ...
  struct VecIntGuiElement : VecTypeGuiElement<int>
  {
    ...
    void make_input(const size_t i) override { ImGui::InputInt(("[" + std::to_string(i) + "]").data(), &value[i]); }
  ...
  struct VecFloatGuiElement : VecTypeGuiElement<float>
  {
    ...
    void make_input(const size_t i) override { ImGui::InputFloat(("[" + std::to_string(i) + "]").data(), &value[i]); }
  ...
  auto is_vector_int = [](std::string_view tname) { return is_vector_type(tname, is_integral); };

  auto is_vector_float = [](std::string_view tname) { return is_vector_type(tname, is_float); };
    ...
    void setup_gui(YAML::Node root)
    {
      for (YAML::iterator iter = root.begin(); iter != root.end(); ++iter)
      {
        if (iter->second.Tag() != "doc")
        {
          ...
          if (is_integral(real_gansta_type_name_fuk))
            elements.push_back(new IntGuiElement(iter));
          if (is_float(real_gansta_type_name_fuk))
            elements.push_back(new FloatGuiElement(iter));
          if (real_gansta_type_name_fuk == "string")
            elements.push_back(new StringGuiElement(iter));
          if (real_gansta_type_name_fuk == "bool")
            elements.push_back(new BoolGuiElement(iter));
          if (is_vector_float(real_gansta_type_name_fuk))
            elements.push_back(new VecFloatGuiElement(iter));
          if (is_vector_int(real_gansta_type_name_fuk))
            elements.push_back(new VecIntGuiElement(iter));
          if (real_gansta_type_name_fuk == "serializable")
            elements.push_back(new SerializableGuiElement(iter));
        }
      }
    }
  ...
  public:
    ...
    SerializableGuiElement(YAML::iterator& node) : GuiElement(node) { setup_gui(this->node); }
  ...
  int EditorWindow::error_code() const noexcept { return cast_error_code(error_code_); }

  int EditorWindow::cast_error_code(const ErrorCode code) const noexcept { return static_cast<int>(code); }

  void EditorWindow::glfw_error_callback(int error, const char* description) { fprintf(stderr, "Glfw Error %d: %s\n", error, description); }
